...
/**
 * That is sum of
 * - https://github.com/leonchen83/redis-cluster-watchdog/blob/31b41eb34576e54dcee73cd8b025caac5d12138d/src/main/java/com/moilioncircle/redis/cluster/watchdog/util/CRC64.java
 * and
 * trackingInvalidateKey function from Redis:
 * <pre>
 *  uint64_t hash = crc64(0,
 *     (unsigned char*)sdskey,sdslen(sdskey))&(TRACKING_TABLE_SIZE-1);
 * </pre>
 *
 * Because we can calulcate exactly the same cache slot as Redis does, we can also invalidate based on
 * received key.
 */
public class CRCUtils {

    private static final long[] CRC64_TAB = {
    ...
    private static long crc64(byte[] bytes, int start, int length) {
        long sum = 0;
        for (int i = start; i < length; i++) {
            sum = (sum >>> 8) ^ CRC64_TAB[((int) sum ^ bytes[i]) & 0XFF];
        }
        return sum;
    }
